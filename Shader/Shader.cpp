...
Shader::Shader(const GLchar* vertexPath, const GLchar* fragmentPath, const char* geometryPath, const char* computePath)
{
	// 1. 从文件路径中获取顶点/片段着色器
	...
	if (computePath != nullptr)
	{
		...
		if (!success)
		{
			glGetShaderInfoLog(compute, 512, NULL, infoLog); std::cout << "ERROR::SHADER::compute::COMPILATION_FAILED\n" << infoLog << std::endl;
		};
	}
	// 着色器程序
	ID = glCreateProgram(); 
